    ...
    if query_videocontroller:
        ...
        for line in query_videocontroller.decode('utf-8').split('\n'):
            if '=' in line:
                ...
                query_videocontroller2 = subprocess.check_output('wmic path win32_videocontroller get PNPDeviceID /format:list')
                if query_videocontroller2:
                    data = {}
                    for line in query_videocontroller2.decode('utf-8').split('\n'):
                        if '=' in line:
                            key, value = line.split('=', 1)
                            data[key] = value.rstrip('\r')
